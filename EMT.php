...
class EMT_Lib
{
        ...
        if ($way)
            $text = preg_replace_callback('/(\<\/?)([^<>]+?)(\>)/s', function ($m) {
                return (strlen($m[1]) == 1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-') ?
                    $m[0] : $m[1] . (substr(trim($m[2]), 0, 1) === "a" ? "%%___" :
                        "") . EMT_Lib::encrypt_tag(trim($m[2])) . $m[3];
            }, $text); else
            $text = preg_replace_callback('/(\<\/?)([^<>]+?)(\>)/s', function ($m) {
                return (strlen($m[1]) == 1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-') ?
                    $m[0] :
                    $m[1] . (substr(trim($m[2]), 0, 3) === "%%___" ? EMT_Lib::decrypt_tag(substr(trim($m[2]), 4)) :
                        EMT_Lib::decrypt_tag(trim($m[2]))) . $m[3];
            }, $text);
        return $text;
    }
    ...
    public static function convert_html_entities_to_unicode(&$text)
    {
        $text = preg_replace_callback("/\&#([0-9]+)\;/", function ($m) {
            return EMT_Lib::_getUnicodeChar(intval($m[1]));
        }, $text);
        $text = preg_replace_callback("/\&#x([0-9A-F]+)\;/", function ($m) {
            return EMT_Lib::_getUnicodeChar(hexdec($m[1]));
        }, $text);
        $text = preg_replace_callback("/\&([a-zA-Z0-9]+)\;/", function ($m) {
            $r = EMT_Lib::html_char_entity_to_unicode($m[1]);
            return $r ? $r : $m[0];
        }, $text);
    }
...
class EMT_Tret
{
    ...
    private function apply_rule($rule)
    {
        ...
        if ($disabled) {
            $this->log("Правило $name", "Правило отключено" . ((isset($rule['disabled']) && $rule['disabled']) ?
                    " (по умолчанию)" : ""));
            return;
        }
            ...
            if (!(isset($rule['pattern']) && $rule['pattern'])) {
                ...
                if (preg_match("/^[a-z_0-9]+$/i", $rule['function'])) {
                    if (method_exists($this, $rule['function'])) {
                        ...
                        $this->_text = preg_replace_callback($rule['pattern'], [
                            $this,
                            $rule['function']
                        ], $this->_text);
                        return;
                    ...
                    $this->_text = preg_replace_callback($rule['pattern'], function ($m) {
                        $rule['function'];
                    }, $this->_text);
    ...
    protected function tag($content, $tag = 'span', $attribute = [])
    {
        ...
        return EMT_Lib::build_safe_tag($content, $tag, $attribute, $this->use_layout === false ? EMT_Lib::LAYOUT_STYLE :
            $this->use_layout);
    }
    ...
    public function activate($list, $disable = false, $strict = true)
    {
        ...
        foreach ($list as $rulename) {
            if ($disable)
                $this->disable_rule($rulename); else $this->enable_rule($rulename);
        }
    ...
    {
        if (is_string($list))
            $rlist = [$list]; elseif (is_array($list))
            $rlist = $list;
...
class EMT_Tret_OptAlign extends EMT_Tret
{
    ...
    {
        $this->_text = $this->preg_replace_e('/(<' . self::BASE64_PARAGRAPH_TAG . '>)([\040\t]+)?(\&laquo\;)/e', '$m[1] . $this->tag($m[3], "span", array("class"=>"oa_oquote_nl"))', $this->_text);
    }
...
class EMT_Tret_Quote extends EMT_Tret
{
    ...
    protected function build_sub_quotations()
    {
        ...
        $exp = strpos($this->_text, "</cA===>") !== false ? "</cA===>" :
            (strpos($this->_text, "\r\n") !== false ? "\r\n\r\n" : "\n\n");
...
class EMT_Base
{
    ...
    public function safe_blocks($text, $way, $show = true)
    {
        if (count($this->_safe_blocks)) {
            ...
            foreach ($safeblocks as $block) {
                $text = preg_replace_callback("/({$block['open']})(.+?)({$block['close']})/s", function ($m) {
    ...
    public function apply($trets = null)
    {
        ...
        $atrets = $this->trets;
        if (is_string($trets))
            $atrets = [$trets]; elseif (is_array($trets))
            $atrets = $trets;
        ...
        foreach ($this->trets as $tret) {
            ...
            foreach ($arr as $classname => $str) {
                ...
                $clsname = ($this->class_layout_prefix ? $this->class_layout_prefix :
                        "") . (isset($this->tret_objects[$tret]->class_names[$classname]) ?
                        $this->tret_objects[$tret]->class_names[$classname] : $classname);
                $res[$clsname] = $str;
            }
        }
        if ($list)
        ...
        if (isset($setupmap['map']) || isset($setupmap['maps'])) {
            ...
            if (is_array($setupmap['maps'])) {
                foreach ($setupmap['maps'] as $map) {
                    $this->set_enable_map($map['map'], isset($map['disable']) ? $map['disable'] :
                        false, isset($map['strict']) ? $map['strict'] : false);
                }
            }
            unset($setupmap['maps']);
        }
...
class EMTypograph extends EMT_Base
{
    ...
    protected $all_options = [
        ...
        'Etc.unicode_convert' => [
            ...
            'selector' => ['*', 'Etc.nobr_to_nbsp'],
            'setting' => ['dounicode', 'active'],
            'exact_selector' => true,
            'disabled' => true
        ],
    ...
    public function get_options_list()
    {
        ...
        foreach ($this->group_list as $group => $ginfo) {
            if ($ginfo === true) {
                ...
                if ($tret)
                    $info['title'] = $tret->title; else $info['title'] = "Не определено";
            } else {
                $info = $ginfo;
            }
            $info['name'] = $group;
